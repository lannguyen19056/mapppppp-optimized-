...
def main():
    ...
    parser.add_argument("--output-index", required=True, help="Path to save the spatial index file (e.g., spatial_index.idx or just spatial_index as base name)")
    ...
    index_basename = args.output_index
    if index_basename.endswith(".idx"):
        index_basename = index_basename[:-4]
    
    idx_props = index.Property()
    spatial_idx_main = index.Index(index_basename, properties=idx_props)
    ...
    print(f"DEBUG: Before closing spatial_index_main: Type={type(spatial_idx_main)}, Number of items inserted={indexed_way_count_main}", file=sys.stderr)
    ...
    spatial_idx_main.close()
    print(f"Spatial index files saved to {index_basename}.idx and {index_basename}.dat", file=sys.stderr)
